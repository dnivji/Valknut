...
class world(): #создаваемый из main.py класс мира, содержащий в себе все сведения об объекте игрового мира
	def __init__(self, size, tile, octaves, seed): #генерация мира по введенным параметрам. Параметры следует перенести куда-то, потому что после генерации мира они уже частично неинтересны
		...
		t1 = time.time()
		pnf = PerlinNoiseFactory(3, self.seed, octaves=self.octaves, tile=(self.space_range, self.space_range, self.frame_range)) #объект трехмерного шума перлина
		...
		def heights_analysis(map_tiles): #объект, записывающий в лог сведения о мире (высоты и таймштампы)
			heigh = [] #создание 
		...
		class ZArray:
			...
			def __str__(self):
				return "ZArray(" + ", ".join([str(i) for i in self._array]) + ")"
